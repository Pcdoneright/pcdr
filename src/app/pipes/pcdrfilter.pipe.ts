...
export class PcdrFilterPipe {
    ...
    private filterByStringExact(filter) {
        return value => {
            // debugger;
            return !filter || (value ? value === filter : false);
        }
    }
    ...
    private filterByObject(filter, pExact) {
        return value => {
            for (let key in filter) {
                ...
                if (type === 'boolean') {
                    ...
                    if (pExact)
                        isMatching = this.filterByStringExact(filter[key])(val);
                    else
                        isMatching = this.filterByString(filter[key])(val);
                } else if (type === 'object') {
                    isMatching = this.filterByObject(filter[key], pExact)(val);
                } else {
    ...
    transform(array: any[], filter: any, exact = false): any {
        ...
        if (type === 'object') {
            return array.filter(this.filterByObject(filter, exact));
        }
